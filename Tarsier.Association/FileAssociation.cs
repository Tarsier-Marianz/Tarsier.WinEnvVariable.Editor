...
namespace Tarsier.Association { /// <summary>List of commands.</summary>
    ...
    public class FileAssociation {
        ...
        public void Create() {
            ...
            try {
                ...
                RegKey.Close();
                RegKey = Registry.ClassesRoot.CreateSubKey(ProgramId);
                RegKey.SetValue("", Description);
                RegKey.Close();
                if(IconPath != "") {
                    RegKey = Registry.ClassesRoot.CreateSubKey(ProgramId + "\\" + "DefaultIcon");
                    RegKey.SetValue("", IconPath + "," + IconIndex.ToString());
                    RegKey.Close();
                }
                for(cnt = 0; cnt < FileInfo.Commands.Captions.Count; cnt++) {
                    RegKey = Registry.ClassesRoot.CreateSubKey(ProgramId + "\\" + "Shell" + "\\" + (String)FileInfo.Commands.Captions[cnt]);
        ...
        public void Remove() {
            if(Extension == null || ProgramId == null)
                throw new ArgumentNullException();
            if(Extension == "" || ProgramId == "")
                throw new ArgumentException();
